...
def _has_grad_tensor(tensors):
    return any(
        torch.is_tensor(tensor) and torch.is_floating_point(tensor) and tensor.requires_grad
...
class _GPTFFModelBase(nn.Module):
    ...
    def set_checkpointing(self, enabled):
        self.use_layer_checkpoint = bool(enabled)
    ...
    def _maybe_checkpoint(self, function, *args):
        if self.use_layer_checkpoint and torch.is_grad_enabled() and _has_grad_tensor(args):
            return torch_checkpoint(function, *args, use_reentrant=False, preserve_rng_state=False)
...
class ThreeBody(nn.Module):
    ...
    def forward(self, atom_fea, edge_ij, triple_dist_ij, triple_dist_ik, triple_a_jik, nbr_atoms, bond_pairs_indices, n_bond_pairs_bond, three_body_indices=None):
        ...
        if three_body_indices is None:
            nbr_i, nbr_j, triple_i_indices, triple_j_indices, triple_k_indices, bond_pair_target = _three_body_indices(
                nbr_atoms, bond_pairs_indices, n_bond_pairs_bond
            )
        else:
            _nbr_i, _nbr_j, triple_i_indices, triple_j_indices, triple_k_indices, bond_pair_target = three_body_indices
...
class tModLodaer_t(_GPTFFModelBase):
    ...
    def forward(self, atom_fea, bonds_r, n_atoms, triple_dist_ij, triple_dist_ik, triple_a_jik, nbr_atoms, n_bond_pairs_bond, bond_pairs_indices):
        ...
        for edge_func, conv, three, transformer, norm1, norm2 in zip(self.edge_updates, self.convs, self.three, self.transformers, self.norms1, self.norms2):
            def layer_block(
                ...
                edge_ij_block,
                bonds_dist_block,
                triple_dist_ij_block,
                triple_dist_ik_block,
                triple_a_jik_block,
                edge_func=edge_func,
                conv=conv,
                three=three,
                ...
                atom_fea_list = pad_sequence(torch.split(atom_next, n_atoms_list), batch_first=True)
                atom_next = norm2(transformer(atom_fea_list, masks))[~masks, :] + atom_next
                return atom_next, edge_next

            atom_fea, edge_ij = self._maybe_checkpoint(
                layer_block,
                atom_fea,
                edge_ij,
                bonds_dist,
...
class tModLodaer(_GPTFFModelBase):
    def __init__(self, CFG
                        ):
